...
function createIconNode({ name, src, color, size, library = 'fas', rotate = 0, classList = '' }) {
	...
	if(typeof src == 'string') {
		...
		// get classes
		classes = getIconNodeClasses(library, name);
	}
	...
	// FontAwesome supports setting 'transform' and 'color' properties
	if(color != undefined)
		el.style.color = `var(--${color}-500)`;
...
function getIconNodeClasses(library, name) {
	switch(library) {
		case 'fab': 
			return ['fa-brands', `fa-${name}`];
		case 'fad': 
			return ['fa-duotone', `fa-${name}`];
		case 'fal':
